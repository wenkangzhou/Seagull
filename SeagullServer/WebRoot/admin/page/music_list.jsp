...
<body>
	...
	<div id="content">
	<div id="main">
		<form id="queryform" action="music.do" method="post">
				...
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
					<tr>
						...
						<td>
							...
							<input value="添加" type="button" class="btn btn-green" onclick="javascript:location='music.do?method=toAdd';">
						</td>
					</tr>
				</table>
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
						...
						<logic:present name="list">
							<logic:iterate id="item" name="list">
								<tr>
									...
									<td>
										<audio src="${item.musicsrc}" controls="controls" style="width:45px"></audio>
									</td>
									<td>
										${item.operator}
									</td>
									<td>
										<a href="music.do?method=toEdit&id=${item.id}">编辑</a>
										<a href="music.do?method=Preview&id=${item.id}" target="_blank">预览</a>
										<c:if test="${sessionScope.admin.channel == 'admin' }">
										<a href="javascript:#;" onclick="javascript:if(confirm('确认删除?'))location='music.do?method=doDelete&id=${item.id}';">删除</a>
										</c:if>
									</td>
								</tr>
							</logic:iterate>
