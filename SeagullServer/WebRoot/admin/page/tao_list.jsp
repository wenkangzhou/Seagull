...
<body>
	...
	<div id="content">
	<div id="main">
		<form id="queryform" action="tao.do" method="post">
				...
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
					<tr>
						...
						<td>
							...
							<input value="添加" type="button" class="btn btn-green" onclick="javascript:location='tao.do?method=toAdd';">
						</td>
					</tr>
				</table>
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
								...
								<tr>
									...
									<td>
										<a href="${item.websrc}" target="_blank">
											<script type="text/javascript">
												document.writeln(csubstr('${item.websrc}',20));
											</script>
										</a>
									</td>
									<td>
										<script type="text/javascript">
											document.writeln(csubstr('${item.etc}',20));
										</script>
									</td>
									<td>
										${item.operator}
									...
									<td>
										<a href="tao.do?method=toEdit&id=${item.id}">编辑</a>
										<a href="tao.do?method=Preview&id=${item.id}" target="_blank">预览</a>
										<c:if test="${sessionScope.admin.channel == 'admin' }">
										<a href="javascript:#;" onclick="javascript:if(confirm('确认删除?'))location='tao.do?method=doDelete&id=${item.id}';">删除</a>
										</c:if>
									</td>
								</tr>
							</logic:iterate>
