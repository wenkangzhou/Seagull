...
<body>
	...
	<div id="content">
	<div id="main">
		<form id="queryform" action="sui.do" method="post">
				...
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
					<tr>
						...
						<td>
							...
							<input value="添加" type="button" class="btn btn-green" onclick="javascript:location='sui.do?method=toAdd';">
						</td>
					</tr>
				</table>
				<table class="fullwidth" cellspacing="0" cellpadding="0" border="0">
						...
						<logic:present name="list">
							<logic:iterate id="item" name="list">
								<tr>
									...
									<td>
										<c:choose>
											<c:when test="${item.coversrc=='#'}">
												<img width="78" height="78" src="img/sys/zhihu.jpg">
											</c:when>
											<c:otherwise>
												<img width="78" height="78" src="${item.coversrc}">	
											</c:otherwise>
										</c:choose>
									</td>
									<td>
										<a href="${item.websrc}" target="_blank">
											<script type="text/javascript">
												document.writeln(csubstr('${item.websrc}',10));
											</script>
										</a>
									</td>					
									<td>
										<script type="text/javascript">
											document.writeln(csubstr('${item.etc}',10));
										</script>
									</td>
									<td>
										${item.type}
									...
									<td>
										<img width="40" height="40" src="${item.typesrc}">		
									</td>
									<td>
										<script type="text/javascript">
											document.writeln(csubstr('${item.title}',10));
										</script>
									</td>	
									<td>
										${item.operator}
									...
									<td>
										<a href="sui.do?method=toEdit&id=${item.id}">编辑</a>
										<a href="sui.do?method=Preview&id=${item.id}" target="_blank">预览</a>
										<c:if test="${sessionScope.admin.channel == 'admin' }">
										<a href="javascript:#;" onclick="javascript:if(confirm('确认删除?'))location='sui.do?method=doDelete&id=${item.id}';">删除</a>
										</c:if>
									</td>
								</tr>
							</logic:iterate>
